...
- ✅ **Saga Pattern / Saga Choreography** untuk kompensasi transaksi antarservice.
- ✅ **Console log rapi** untuk bukti alur sukses dan gagal.
- ✅ **Business failure** melalui `payment.failed` dan `shipment.failed` topic.
...
## 🔁 Saga Pattern / Saga Choreography

Versi ini juga menambahkan **Saga Pattern berbasis choreography**. Pada pendekatan ini, tidak ada satu database transaction besar yang mengatur semua service. Setiap service melakukan transaksi lokal pada database masing-masing, lalu menerbitkan event Kafka untuk memicu langkah berikutnya.

### ✅ Saga Sukses

```text
USER Create Order
→ order-service menyimpan order PENDING_PAYMENT
→ inventory-service reserve stock
→ order-service publish Order Created Event
→ payment-service memproses payment PAID
→ payment-service publish Payment Success Event
→ shipping-service membuat shipment SHIPPED
→ shipping-service publish Shipment Created Event
→ notification-service mencatat notifikasi
```

### ❌ Saga Compensation 1: Payment Failed
...
inventory-service     → Release stock sebagai kompensasi
notification-service  → Simpan notifikasi PAYMENT_FAILED
```

Output terminal utama:

```text
============================================================
SAGA - PAYMENT FAILED, COMPENSATION REQUIRED
============================================================
Saga ID        : SAGA-2026-0002
Order ID       : 2
...
### ❌ Saga Compensation 2: Shipment Failed

Gunakan:

```json
{
  "shippingAddress": "SAGA_FAIL_SHIPPING",
  "paymentMethod": "BCA",
  "items": [
    {
      "sku": "VLR-TSH-BLK-M",
...
```text
payment-service       → Payment awal PAID, lalu REFUNDED
shipping-service      → Shipment status FAILED
order-service         → Order status CANCELLED
inventory-service     → Release stock sebagai kompensasi
notification-service  → Simpan SHIPMENT_FAILED dan PAYMENT_REFUNDED
```
...
### 📌 Topic Kafka Tambahan untuk Saga

| Topic | Producer | Consumer | Fungsi |
|---|---|---|---|
| `velora.shipment.failed.topic` | shipping-service | order-service, inventory-service, payment-service, notification-service | Event saat shipment gagal secara bisnis dan memicu kompensasi Saga |
| `velora.payment.refunded.topic` | payment-service | notification-service | Event saat payment direfund sebagai kompensasi dari shipment failed |

### 📌 Perbedaan Business Failure, Technical Failure, dan Saga

| Skenario | Trigger | Jenis | Hasil |
|---|---|---|---|
| Payment failed | `paymentMethod: "FAIL"` | Business failure + Saga compensation | Payment FAILED, order PAYMENT_FAILED, stock dirilis |
| Payment DLQ | `paymentMethod: "DLQ"` | Technical failure | Message masuk `velora.order.topic.DLQ` |
| Shipping failed Saga | `shippingAddress: "SAGA_FAIL_SHIPPING"` | Business failure + Saga compensation | Shipment FAILED, payment REFUNDED, order CANCELLED, stock dirilis |
| Shipping DLQ | `shippingAddress: "FAIL-SHIPPING"` | Technical failure | Message masuk `velora.payment.success.topic.DLQ` |

> Jadi: **Saga Pattern** dipakai untuk kegagalan bisnis yang membutuhkan kompensasi, sedangkan **DLQ** dipakai untuk kegagalan teknis saat consumer gagal memproses message.
